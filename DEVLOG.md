...
1. 应用户「要专业、带真实产品截图」要求改版详情页：
   ...
   - Hero 右侧：有 `icon` 时显示真实 app 图标（drop-shadow）+ 规格芯片（版本 / macOS 13+ · Apple Silicon / DMG 约 326 MB），替代卡通 illo；无 icon 的产品回退原 illo 逻辑。**二轮收敛（用户反馈 logo 占地太大）**：改为单列紧凑 hero（`pdp-hero-compact`），54px 小图标内联标题左侧，版本/平台/体积/签名合并为一行 meta；根因是 `.pdp-hero-illo img { width:100%; max-width:420px }` 特异性压过图标尺寸类，把 logo 放大到 420px。**三轮调序（用户要求）**：FEATURES（核心能力）移到 SHOWCASE 截图之前。**四轮定稿（用户反馈全宽截图太大）**：hero 恢复双栏——左 logo+文案、右 gallery 首张截图（`pdp-hero-shot`，420px 缩小版 + 柔投影），hero 用掉的截图不再进下方 Showcase（`restGallery`），后续补图自动落入画廊；无截图的 icon 产品保持单列紧凑。最终顺序 Hero(左logo右截图) → 核心能力 → 安装 → 说明 → Stack。
...
## 2026-08-29 会话记录（首页排版重构）

### 今天完成

1. **首页排版重构**（用户反馈「排版很丑」）：
   - 新增 **Hero 开场**：衬线大标题「在 AI 的浪里，搭一座慢站点」（复用页脚品牌语，新增 I18N 键 home_hero_title）+ 一句定位（复用 home_intro_lead）+ 三个动作（浏览全部文章 / 关于我 / 🤖试试 AI 问答 chip，复用旧 intro 键）。
   - 工具栏重做：左侧「最新文章」小标签（新键 home_list_label，大写字距）+ 右侧视图切换，底部长发界线——切换按钮不再孤悬。
   - 列表视图：封面图缩为固定 200px 列（`:has()` 只作用于带图卡片，纯文字卡通栏）；标题 hover 变品牌橙。
   - 网格视图：卡片加 `--color-surface-cream-strong` 底 + 圆角成真卡片；`align-items: start` 消除短卡拉伸留白；auto-fill 三列自适应。
